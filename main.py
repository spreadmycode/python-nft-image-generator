...
cloth_weights = [31.66, 7.4, 5.5, 4.9, 6.8, 6.3, 4.8, 3.2, 3.4, 2.6, 3.1, 3.1, 3.6, 2.9, 2.5, 1.7, 1.5, 2.4, 0.25, 0.8, 0.33, 0.29, 0.3, 0.67]
cloth_files = {
    "None": "None.png",
...
eyewear_weights = [31.59, 5.18, 4.72, 4.26, 4.2, 4.05, 3.74, 3.63, 3.32, 3.09, 2.97, 2.86, 2.76, 2.54, 2.40, 2.29, 2.26, 2.23, 2.11, 2.04, 2.01, 1.87, 1.79, 0.45, 0.44, 0.34, 0.23, 0.2, 0.17, 0.15, 0.11]
eyewear_files = {
    "None": "None.png",
...
def create_new_image():
    ...
    new_image ["Clothes"] = random.choices(clothes, cloth_weights)[0]
    new_image ["Earring"] = random.choices(earrings, earring_weights)[0]
    new_image ["Eyes"] = random.choices(eyewears, eyewear_weights)[0]
    new_image ["Fur"] = random.choices(furs, fur_weights)[0]
    new_image ["Hat"] = random.choices(hats, hat_weights)[0]
    new_image ["Mouth"] = random.choices(mouthes, mouth_weights)[0]

    # Constraints for combination
    if (new_image["Clothes"] == "Solana Hoodie" or new_image["Clothes"] == "Skeleton Outfit") and (new_image["Hat"] != "None" or new_image["Earring"] != "None"):
    ...
    if (new_image["Hat"] == "Black Durag" or new_image["Hat"] == "Rainbow Durag") and new_image["Eyes"] == "Cyborg Spider":
        return create_new_image()
    
    if new_image["Hat"] == "Chief Helmet" and (new_image["Mouth"] != "None" or new_image["Eyes"] != "None"):
        return create_new_image()
